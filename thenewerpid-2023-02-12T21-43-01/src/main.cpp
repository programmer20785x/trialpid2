...
// LeftMotor            motor_group   14, 15          
// RightMotor           motor_group   19, 20          
...
int drivePID(){
  ...
  while(enableDrivePID){
    ...
    //Potential
    turnError = desiredTurnValue - Inertial2.heading() ;
...
void autonomous(void) {
  ...
  vex::task runpid(drivePID);

  resetDriveSensors = true;
  desiredValue = 300; // drive fwd for 300 motor rotations i hope??
  waitUntil(error< 2);
  desiredTurnValue = 90; // turn 90 degrees i hope??
  desiredValue = -300;//// drive back 300 notor rotations

  vex::task::sleep(1000);

  resetDriveSensors = true; /// reset position only (might remove)
  desiredValue = 300;
